...
/// Campfire 同步补丁。
/// </summary>
public static class CampfireSyncPatch
{
	...
	private static readonly Dictionary<string, Queue<CampfireEventSnapshot>> RoomEventsByRoomKey = new(StringComparer.Ordinal);
	...
	public static void BroadcastCampfireEvent(string eventType, string cardId = null, string cardName = null)
	{
		if (!IsCampfireEvent(eventType))
		{
			return;
		}
		...
		CampfireEventSnapshot snapshot = new()
		{
			ActionId = actionId,
			EventType = eventType,
	...
	{
		if (!IsCampfireEvent(eventType) || !TryGetJsonElement(payload, out JsonElement root))
		{
			return;
		}
		...
		AppendRoomEvent(new CampfireEventSnapshot
		{
			ActionId = actionId,
			EventType = eventType,
			PlayerId = playerId,
		...
		Plugin.Logger?.LogInfo($"[CampfireSync] recv {eventType}: player={playerId}, card={cardName}/{cardId}, actionId={actionId}");
	}

	/// <summary>
	/// 获取指定房间最近 Campfire 事件（默认最多 8 条）。
	/// </summary>
	public static List<CampfireEventSnapshot> GetRecentRoomEvents(string roomKey, int maxCount = MaxRoomEventsPerRoom)
	{
		...
		lock (RoomEventsLock)
		{
			if (!RoomEventsByRoomKey.TryGetValue(roomKey, out Queue<CampfireEventSnapshot> queue) || queue.Count == 0)
			{
				return [];
			}

			List<CampfireEventSnapshot> events = [];
			foreach (CampfireEventSnapshot e in queue)
			{
	...
	/// <summary>
	/// 合并中途加入追赶下发的 Campfire 事件：仅缓存并按 ActionId 去重标记，不执行游戏动作。
	/// </summary>
	public static void MergeCatchupEvents(string roomKey, IReadOnlyList<CampfireEventSnapshot> events)
	{
		...
		{
			CampfireEventSnapshot e = events[i];
			if (e == null || string.IsNullOrWhiteSpace(e.EventType) || !IsCampfireEvent(e.EventType))
			{
				continue;
			}
	...
	private static bool IsCampfireEvent(string eventType)
	{
		return string.Equals(eventType, NetworkMessageTypes.CampfireUpgradeSelected, StringComparison.Ordinal) ||
			   string.Equals(eventType, NetworkMessageTypes.CampfireRemoveCard, StringComparison.Ordinal) ||
	...
	private static void AppendRoomEvent(CampfireEventSnapshot snapshot)
	{
		if (snapshot == null || string.IsNullOrWhiteSpace(snapshot.RoomKey) || !IsCampfireEvent(snapshot.EventType))
		{
			return;
		}
		...
		lock (RoomEventsLock)
		{
			if (!RoomEventsByRoomKey.TryGetValue(snapshot.RoomKey, out Queue<CampfireEventSnapshot> queue))
			{
				queue = new Queue<CampfireEventSnapshot>();
	...
	private static CampfireEventSnapshot CloneEvent(CampfireEventSnapshot e)
	{
		return new CampfireEventSnapshot
		{
