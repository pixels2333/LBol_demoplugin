...
2) 将中途加入/重连的核心同步切换为：
   ...
   - 开局规则对齐（Difficulty/Puzzles/GameMode/Stages/DebutAdventure），保证 joiner 可自选角色但地图与难度一致
...
7) 中途加入者（joiner）允许自行选择角色/流派，但以下内容必须与主机一致：
   - Difficulty
   - RootSeed（从而保证 Stage.MapSeed/UISeed 链路一致）
   - Stages 列表与顺序（保证 RootRng 对 Stage.MapSeed 的消耗一致）
...
业务约束:
  ...
  - 不发送 GameRunSaveData 存档文件
   - joiner 可自选角色，但 Difficulty/Seed/Stages 必须与主机一致
```
...
为支持“joiner 自选角色但难度/地图一致”，建议扩充：
- FullStateSnapshot.GameState: Difficulty/Puzzles/GameMode/ShowRandomResult
- FullStateSnapshot.GameState: StageTypeName 列表（按顺序）
- FullStateSnapshot.GameState: DebutAdventureTypeName（用于 stage0 的 DebutAdventureType）
...
4) 客户端打开“新游戏/角色选择”界面，让加入者自行选择角色
   - 但会强制锁定为主机一致的 Difficulty/Seed/Stages（保证 MapSeedUlong 可对齐）
5) 客户端创建新 GameRun 并停留在地图界面
