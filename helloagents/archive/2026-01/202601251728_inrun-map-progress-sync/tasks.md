...
- [ ] 4.2 新加入：创建本地 GameRun 并对齐 seeds
  ...
  - 需要确认的技术点:
    - joiner 是否必须走“新开局/选角色”流程（结论：是；joiner 可自选角色，但其他参数必须锁定为主机一致）
    - 如何在 StartGamePanel/StartGame 流程中强制 Seed/Difficulty/Stages 与主机一致
  - 验证:
    - 生成的地图与主机一致（MapSeedUlong 一致）

- [ ] 4.2.1 扩充 FullSnapshot 开局对齐字段（Host -> Client）
  - 目标: joiner 在自选角色时仍可强制与主机一致的开局参数
  - 建议新增字段（挂在 GameStateSnapshot 或新增 HostStartConfigSnapshot）：
    - Difficulty
    - Puzzles
    - GameMode
    - ShowRandomResult
    - StageTypeNames（按顺序）
    - DebutAdventureTypeName（用于 stage0）
  - 验证:
    - joiner 收到 FullSnapshot 后可以构造 StartGameData.StagesCreateFunc（按 host 的 StageTypeNames）
    - joiner 开局后 run.CurrentStage.MapSeed == snapshot.MapState.MapSeedUlong

- [ ] 4.2.2 joiner 打开 StartGamePanel 进行选角（锁定主机参数）
  - 目标: joiner 可选角色，但 Seed/Difficulty/Puzzles/GameMode/Stages 强制一致
  - 建议实现:
    - joiner 收到 FullSnapshot 且本地无 GameRun 时：自动打开 StartGamePanel（或提示并一键打开）
    - 在 StartGamePanel 界面展示“联机追赶中：难度/种子已锁定为房主设置”
    - 对 ConfirmStart 加 Harmony Patch：把 seed/difficulty/puzzles/stagesCreateFunc 强制替换为 host 值
  - 验证:
    - joiner 无需手动输入 seed；点击开始即可进入地图并触发 MapCatchUp
