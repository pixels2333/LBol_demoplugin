# 任务清单: midgamejoin_todo

目录: `helloagents/plan/无_midgamejoin_todo/`
...
- [-] 4.2 在 `networkplugin/Network/MidGameJoin/MidGameJoinManager.cs` 接入 `GenerateStartingCards/GenerateStartingExhibits/GenerateStartingPotions` 并挂到 `ApproveJoin()`（受 `EnableCompensation` 控制），验证 why.md#需求-进度计算与补偿生成
  > 备注: 当前补偿生成函数为安全起见返回空集合（避免生成非法 ID）；后续可接入白名单/游戏数据校验后再启用真实补偿。
