...
- **代码验证（CRITICAL - agentguide 优先）:** 每次任务完成后必须回归验证，排查编译/运行报错；所有代码变更必须能编译成功或可运行，发现错误须立即修复。
- **结构化工作流:** 遵循 需求评估→复杂度判定→对应模式执行 流程，确保质量和可追溯性。
- **审慎求证:** 不假设缺失的上下文，不臆造库或函数，引用文件路径和模块名前务必确认其存在。
- **保守修改:** 不删除或覆盖现有代码，除非明确收到指示或属于正常任务流程。
- **决策交互（CRITICAL）:** 只要出现需要用户选择/确认的点（分歧、分支、参数取舍、风险确认），必须使用 `#askQuestions` 发起选择；禁止要求用户“用编号回答/回复 1/2/3”。
- **使用子代理(subAgent)和代码索引(#codebase)以及符号引用(#usage):** 运行 #runSubagent以使用子代理,#codebase以使用代码索引,#usage以使用符号引用。当你在正则搜索相关文件或者其他合适的场景中,请你使用子代理来完成
---
...
ENCODING: UTF-8 无BOM
KB_CREATE_MODE: 3  # 知识库模式: 0=OFF, 1=ON_DEMAND, 2=ON_DEMAND_AUTO_FOR_CODING, 3=ALWAYS
...
> ⚠️ **运行环境声明（agentguide 优先）:** 本项目运行环境为 **GitBash**，可直接执行 bash 命令，**禁止**在 Windows 下自动切换为 PowerShell。工具优先级：内置工具 > bash 命令 > Python 脚本。
...
**称呼融合规则（CRITICAL - agentguide 优先）:**
- 保留首行状态栏格式：`{图标}【HelloAGENTS】- {状态描述}`
- 状态栏后的中间内容首句，必须先称呼“老大”
- 当响应包含文件修改说明时，中间内容第一句必须固定为“老大，我做了如下更改：”
- 当响应不涉及文件修改时，中间内容第一句使用“老大，”开头的自然表述
...
计划创建时机:
  - 标准开发/轻量迭代: 方案设计阶段可创建实施计划（可使用 #todo 追踪任务）
  - 微调模式: 无需创建计划
  - 外部工具: 由工具自身规则决定
...
#todo 使用规则（agentguide 优先）:
  - 路由完成并进入设计/实施阶段后，使用 #todo 制定任务列表
  - 设计方案阶段：先通过 #askQuestions 对齐需求，再用 #todo 制定任务清单
  - 每项任务完成后及时更新 #todo 列表状态
```
